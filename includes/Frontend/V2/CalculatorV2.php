...
namespace CCSCalculator\Includes\Frontend\V2;
...
/**
 * CalculatorV2 — the redesigned (premium 3-column) Child Care Subsidy calculator.
 *
 * Delivered as a fully self-contained parallel module: it registers its own
 * shortcode [ccs_calculator_v2] and enqueues its own CSS/JS, and shares the
 * existing AJAX endpoints (ccs_search_suburbs / send_summary_email /
 * ccs_refresh_nonce, all on the 'ccs_frontend' nonce) and the same
 * childcare_ccs_policy + brand options. The original Shortcode class and its
 * [thechildcare_ccs_calculator] shortcode are left completely untouched, so the
 * live calculator keeps working while this one is developed and previewed.
 */
class CalculatorV2
{
    public function register()
    {
        add_shortcode('ccs_calculator_v2', [$this, 'render']);
        add_action('wp_enqueue_scripts', [$this, 'enqueue_assets']);
    }

    /**
     * Enqueue the v2-only stylesheet/script. The base structural CSS and the
     * admin brand colours are already loaded globally by the shared Assets
     * class (handle: childcare-ccs-style), so these only add the new
     * 3-column shell, sidebar, summary panel, modals and comparison styling.
     */
    public function enqueue_assets()
    {
        ...
        wp_enqueue_style(
            'ccs-v2-style',
            CCS_CALCULATOR_PLUGIN_URL . 'assets/v2/css/ccs-v2.css',
            ['childcare-ccs-style'],
            $cache_buster
        );
        wp_enqueue_script(
            'ccs-v2-script',
            CCS_CALCULATOR_PLUGIN_URL . 'assets/v2/js/ccs-v2.js',
            ['jquery'],
            $cache_buster,
            true
        );
...
<div id="childcare-ccs-calculator" class="childcare-ccs-root ccs-redesign">
  <div class="ccs-shell">

    <!-- ===== Left sidebar: brand + vertical step navigation ===== -->
    <aside class="ccs-sidebar">
      <div class="ccs-brand">
        <span class="ccs-brand-mark" aria-hidden="true">
        ...
        <span class="ccs-brand-text">
          <span class="ccs-brand-title">Childcare Subsidy</span>
          <span class="ccs-brand-sub">Calculator</span>
        </span>
      </div>
      <nav class="ccs-nav" aria-label="Calculator steps">
        <button type="button" class="ccs-nav-item" data-step="1">
          <span class="ccs-nav-ic" aria-hidden="true">1</span>
          <span class="ccs-nav-tx"><span class="ccs-nav-title">Location</span><span class="ccs-nav-desc">Where you live</span></span>
        </button>
        <button type="button" class="ccs-nav-item" data-step="2">
          <span class="ccs-nav-ic" aria-hidden="true">2</span>
          <span class="ccs-nav-tx"><span class="ccs-nav-title">Household Income</span><span class="ccs-nav-desc">Income &amp; activity</span></span>
        </button>
        <button type="button" class="ccs-nav-item" data-step="3">
          <span class="ccs-nav-ic" aria-hidden="true">3</span>
          <span class="ccs-nav-tx"><span class="ccs-nav-title">Children</span><span class="ccs-nav-desc">Attendance &amp; fees</span></span>
        </button>
        <button type="button" class="ccs-nav-item" data-step="4">
          <span class="ccs-nav-ic" aria-hidden="true">4</span>
          <span class="ccs-nav-tx"><span class="ccs-nav-title">Summary</span><span class="ccs-nav-desc">Your estimate</span></span>
        </button>
      </nav>
      <div class="ccs-sidebar-foot">
        <div class="ccs-note-head">
          <span class="ccs-note-ic" aria-hidden="true">
          ...
          <span class="ccs-note-title">Disclaimer</span>
        </div>
        <p class="ccs-note-text"><?php echo esc_html($policy['disclaimer_text'] ?? ''); ?></p>
        <p class="ccs-note-updated">
    ...
    <main class="ccs-main">
      <div class="ccs-main-inner">

    <!-- Enhanced Progress bar (hidden in redesign; still drives step state via JS) -->
    ...
    <div class="childcare-step" id="step1">
        <div class="ccs-step-head">
            <h4 class="ccs-step-title">Your location</h4>
            <p class="ccs-step-desc">Tell us where you live and your current enrolment situation.</p>
        </div>
    ...
    <div class="childcare-step" id="step3" style="display:none;">
        <div class="ccs-step-head">
            <h4 class="ccs-step-title">Children &amp; attendance</h4>
            <p class="ccs-step-desc">Add each child in care, then set their days, hours and daily fee.</p>
        </div>
        <!-- Legacy count-buttons container kept (hidden) only as a validation anchor -->
        <div id="child-buttons" style="display:none;"></div>
        <div id="children-details"></div>
        <div class="ccs-child-actions">
            <button type="button" id="ccs-add-child" class="ccs-add-child">
                <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M12 5v14M5 12h14"/></svg>
                Add another child
            </button>
            <p class="next-steps">
    ...
    <div class="childcare-step" id="step4" style="display:none;">
        ...
        <!-- 2025-26 vs 2026-27 comparison (built by ccs-v2.js when results are ready) -->
        <div id="ccs-comparison" class="ccs-cmp-wrap"></div>

        <!-- Info Box and CTA Buttons Container -->
        <?php if(get_option('ccs_info_box_enabled', 1) || get_option('ccs_cta_section_enabled', 1)): ?>
        <div class="ccs-cta-wrap" style="margin-top:18px; padding:20px; background:#f9f9f9; border-radius:12px;">
            ...
            <div class="ccs-cta-block" style="text-align:center;">
                <h4 class="ccs-cta-title"><?php echo esc_html(get_option('ccs_cta_title', 'Ready to take the next step?')); ?></h4>
                <p class="ccs-cta-desc"><?php echo esc_html(get_option('ccs_cta_desc', 'Book a tour of our centre or get in touch — our team is here to help with your enrolment.')); ?></p>
                <div class="ccs-cta-btns" style="display:flex; gap:12px; justify-content:center; flex-wrap:wrap;">
                    <?php
    ...
    <aside class="ccs-summary-panel" aria-label="Your live estimate">
      <div class="ccs-panel-actions">
        <button type="button" id="ccs-panel-email" class="ccs-panel-btn ccs-panel-btn--email" disabled aria-label="Email your estimate">
        ...
        <button type="button" id="ccs-panel-print" class="ccs-panel-btn ccs-panel-btn--print" disabled aria-label="Print or download your estimate">
      ...
      <div class="ccs-panel-body">
        <div class="ccs-panel-placeholder">
          <span class="ccs-panel-ph-ic" aria-hidden="true">
            <svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"><path d="M3 3v18h18"/><path d="m19 9-5 5-4-4-3 3"/></svg>
          </span>
          <p class="ccs-panel-ph-title">Your live estimate</p>
          <p class="ccs-panel-ph-text">Complete the steps and your Child Care Subsidy estimate will appear here.</p>
        </div>
        <!-- #summary-main-card + key figures are relocated here by ccs-v2.js once results are calculated -->
      </div>
    </aside>
...
jQuery(document).ready(function($){
    ...
    // v2 redesign: initialise the email form up-front so the panel's Email modal
    // always has a ready form to display (original flow also re-inits after step 3).
    ...
    $('#suburb').on('input', function(){
        ...
        $('#suburb-suggestions .suburb-suggestion, #suburb-suggestions .no-results, #suburb-suggestions .error-message').remove();
    ...
    // Child count buttons
    /* ===== Children: accordion cards + add / remove (v2 redesign) ===== */
    const CCS_MAX_CHILDREN = 5;
    function ccsCareOptions(){
        // This provider only offers Centre Based Day Care.
        return '<option value="cbdc" selected>Centre Based Day Care</option>';
    }
    function ccsDayButtons(week){
        const days = [['Mon','Monday','M'],['Tue','Tuesday','T'],['Wed','Wednesday','W'],['Thu','Thursday','T'],['Fri','Friday','F']];
        return days.map(function(d){
            return '<button type="button" class="fortnight-day-btn ccs-day" data-week="'+week+'" data-day="'+d[0]+'">'
                 + '<span class="ccs-day-s">'+d[2]+'</span><span class="ccs-day-f">'+d[1]+'</span></button>';
        }).join('');
    }
    function ccsBuildChildCard(n){
        return ''
        + '<div class="child-details ccs-child-card is-open" data-child="'+n+'">'
          + '<div class="ccs-child-head" tabindex="0" role="button" aria-expanded="true">'
            + '<span class="ccs-child-head-l"><span class="ccs-child-avatar">'+n+'</span>'
              + '<span class="ccs-child-titles"><span class="ccs-child-title">Child '+n+'</span>'
              + '<span class="ccs-child-sub">Tap to edit details</span></span></span>'
            + '<span class="ccs-child-head-r">'
              + '<button type="button" class="ccs-child-remove" aria-label="Remove child" title="Remove child"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round"><path d="M18 6 6 18M6 6l12 12"/></svg></button>'
              + '<span class="ccs-child-chev" aria-hidden="true"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="m6 9 6 6 6-6"/></svg></span>'
            + '</span>'
          + '</div>'
          + '<div class="ccs-child-body">'
            + '<div class="ccs-field-grid">'
              + '<label class="ccs-field"><span class="ccs-field-lb">Date of Birth</span><input type="date" class="child-dob"></label>'
              + '<label class="ccs-field"><span class="ccs-field-lb">Type of Care</span><select class="child-care-type">'+ccsCareOptions()+'</select></label>'
            + '</div>'
            + '<div class="ccs-days">'
              + '<div class="ccs-days-wk"><span class="ccs-days-lb">Week 1</span><div class="ccs-days-row">'+ccsDayButtons(1)+'</div></div>'
              + '<div class="ccs-days-wk"><span class="ccs-days-lb">Week 2</span><div class="ccs-days-row">'+ccsDayButtons(2)+'</div></div>'
            + '</div>'
            + '<div class="ccs-hours-block">'
              + '<span class="ccs-field-lb">Hours per day</span>'
              + '<div class="slider-container ccs-slider">'
                + '<span class="ccs-slider-cap">9</span>'
                + '<div class="ccs-slider-track"><input type="range" class="child-hours-slider" min="9" max="12" value="10" step="0.5"></div>'
                + '<span class="ccs-slider-cap">12</span>'
                + '<input type="number" class="ccs-hours-num" min="9" max="12" step="0.5" value="10" aria-label="Hours per day">'
                + '<span class="ccs-hours-unit">hrs</span>'
              + '</div>'
            + '</div>'
            + '<label class="ccs-field ccs-fee-field"><span class="ccs-field-lb">Fees per day ($)</span><input type="number" class="child-fee" step="0.01" placeholder="0.00"></label>'
            + '<div class="child-calculation-output" style="display:none;"><span class="calc-hours">10</span><span class="calc-days">0</span><span class="calc-fee">0.00</span><span class="calc-total">0.00</span></div>'
          + '</div>'
        + '</div>';
    }
    function ccsRenumberChildren(){
        const $cards = $('#children-details .child-details');
        numChildren = $cards.length;
        $cards.each(function(idx){
            const n = idx + 1;
            $(this).attr('data-child', n);
            $(this).find('.ccs-child-title').first().text('Child ' + n);
            $(this).find('.ccs-child-avatar').first().text(n);
        });
        $('#children-details .ccs-child-remove').css('display', numChildren > 1 ? '' : 'none');
        const atMax = numChildren >= CCS_MAX_CHILDREN;
        $('#ccs-add-child').prop('disabled', atMax).toggleClass('is-max', atMax);
    }
    function ccsAddChild(){
        if (numChildren >= CCS_MAX_CHILDREN) { return; }
        const $card = $(ccsBuildChildCard(numChildren + 1));
        $('#children-details .ccs-child-card').removeClass('is-open').find('.ccs-child-head').attr('aria-expanded','false');
        $('#children-details').append($card);
        ccsRenumberChildren();
        $card.find('.child-hours-slider').trigger('input'); // init slider progress + live calc
        var el = $card[0];
        if (el && el.scrollIntoView) { el.scrollIntoView({behavior:'smooth', block:'nearest'}); }
    }
    $('#ccs-add-child').on('click', ccsAddChild);
    $('#children-details').on('click', '.ccs-child-remove', function(e){
        e.stopPropagation();
    ...
    $('#children-details').on('click', '.ccs-child-head', function(e){
        if ($(e.target).closest('.ccs-child-remove').length) { return; }
        const $card = $(this).closest('.child-details');
        const open = $card.hasClass('is-open');
        $('#children-details .ccs-child-card').removeClass('is-open').find('.ccs-child-head').attr('aria-expanded','false');
        if (!open) { $card.addClass('is-open'); $(this).attr('aria-expanded','true'); }
    });
    $('#children-details').on('keydown', '.ccs-child-head', function(e){
        if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); $(this).trigger('click'); }
    });
    // day toggle (document-level handler recalculates after this)
    $('#children-details').on('click', '.fortnight-day-btn', function(){
        $(this).toggleClass('active');
    });
    // start with one child card
    ccsAddChild();
